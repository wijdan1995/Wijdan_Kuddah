...
def register(request):
    if request.POST:
        ...
        if len(errors) > 0:
            ...
            logged_user = User.objects.create(name=request.POST['name'], alias=request.POST['alias'], email=request.POST['email'], password=pw_hash)
            request.session['user_name'] = logged_user.name
            request.session['user_alias'] = logged_user.alias
